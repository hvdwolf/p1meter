...
# to pyimagefuser where the events are evaluated
    ...
    mainLayout = [ [sg.Text('Stroomverbruik', font = ('any', 12, 'bold')), sg.Button('Vandaag'), sg.CalendarButton('Op datum', key='_CALENDAR_', target='-CAL-', format=( '%Y-%m-%d'),locale='nl_NL',), sg.Text('Gasverbruik', font = ('any', 12, 'bold')), sg.Button('Vandaag', key='_gasvandaag_'), sg.CalendarButton('Op datum', key='_GCALENDAR_', target='-GCAL-', format=( '%Y-%m-%d'),locale='nl_NL',), sg.Push(), sg.Button('Selecteer database'), ],
    ...
    # The sg.In(key='-WEB-' is used in the web scripts and is therefore always empty here.
